    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        for(var y = 0 ; wcpcartdata.items.length > y ; y++){                 
          if(item.id == wcpcartdata.items[y].variant_id){                   
            item.price = wcpcartdata.items[y].wcp_v_price;  
            item.compare_at_price = wcpcartdata.items[y].wcp_v_compare_price;  
            item.final_line_price = wcpcartdata.items[y].wcp_v_price *  wcpcartdata.items[y].quantity;  
          }
        }
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            var WCP_JSON=[];
            if(document.getElementsByClassName('wcp_json_'+data.id).length){
              WCP_JSON=JSON.parse(document.getElementsByClassName('wcp_json_'+data.id)[0].innerText);
            	data.price = WCP_JSON.price;
            	data.compare_at_price = WCP_JSON.compare_at_price;
            }
